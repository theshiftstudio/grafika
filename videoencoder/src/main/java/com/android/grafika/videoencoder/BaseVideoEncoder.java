...
public abstract class BaseVideoEncoder implements Runnable, EncoderStateCallback {
    ...
    protected static final int MSG_PAUSE_RECORDING = 6;
    protected static final int MSG_RESUME_RECORDING = 7;
    ...
    protected EncoderCore mVideoEncoder;
    ...
    protected static class EncoderHandler extends Handler {
        protected WeakReference<BaseVideoEncoder> mWeakEncoder;
     ...
     * Starts recording.
     */
    protected void handleStartRecording(VideoEncoderConfig config) {
    ...
    protected void handleResumeRecording() {
        if (mVideoEncoder.getPauseResumeSupported()) {
     ...
     */
    protected abstract void handleFrameAvailable(float[] transform, long timestampNanos);
    /**
     ...
     */
    protected void handleSetTexture(int id) {
     ...
     */
    protected void handleUpdateSharedContext(EGLContext newSharedContext) {
    ...
    protected abstract EncoderCore createEncoder(VideoEncoderConfig config)
            throws IllegalStateException, IOException;

    protected void prepareEncoder(VideoEncoderConfig config) {
        try {
            mVideoEncoder = createEncoder(config);
     ...
     */
    protected void drawBox(int posn) {
